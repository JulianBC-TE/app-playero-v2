...
import { actualizarPersonaLocal } from "@DBmodules/personaDB";
...
export function EditarPersona({
  ...
  async function handleEditPerson(data: { nombre_apellido: string }) {
    try {
      ...
      await actualizarPersonaLocal(Number(cedula), { nombre_apellido: nombre });
      setIsLoading(false);
  ...
  return (
    <View className="flex-1">
      ...
      <View className="flex-1 p-4 gap-4 items-center">
        <InputCard title="Cedula:" required={false}>
        ...
        <InputCard title="Nombre y Apellido:" required>
          <View className="flex-row w-full items-center">
            <Controller
              control={control}
